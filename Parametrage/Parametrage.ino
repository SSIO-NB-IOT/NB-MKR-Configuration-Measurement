...
void setup() {
  ...
  Serial.println("4G Narrowband Radio Access Technology (RAT) and eDRX configuration.");
  Serial.println();
  Serial.println("You can choose among CAT-M1, NB-IoT or a combination of both for RAT.");
  Serial.println("For eDRX, you can choose between different parameters.");
  Serial.println();
  ...
  Serial.println();
  Serial.println("In order to change the RAT or eDRX, you will need to run this sketch again.");
  for (int i = 0; i < 80; i++) Serial.print("*");
...
void loop() {
  ...
  Serial.println("Please choose your eDRX configuration:");
  Serial.println();
  Serial.println("    0 - Disable eDRX");
  Serial.println("    1 - Enable eDRX with TAU of 10.24s");
  Serial.println("    2 - Enable eDRX with TAU of 20.48s");
  Serial.println("    3 - Enable eDRX with TAU of 40.96s");
  Serial.println("    4 - Enable eDRX with TAU of 61.44s");

  Serial.print("> ");
  while (Serial.available() == 0);
  edrxMode = Serial.parseInt();
  Serial.println(edrxMode);
  ...
  setRAT(uratChoice, edrxChoice);
  apply();

  Serial.println();
  Serial.println("Radio Access Technology and eDRX selected.");
  Serial.println("Now you can upload your 4G application sketch.");
  while (true);
}

bool setRAT(String choice, String edrx) {
  String response;
  ...
  MODEM.sendf("AT+CEDRXS=%s", edrx.c_str());
  MODEM.waitForResponse(2000, &response);
  Serial.println("done.");
